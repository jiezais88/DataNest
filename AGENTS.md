...
- **E2E 测试快速验证快速修复纪律（2026-08-15 起，全局纪律）**：E2E 用例失败后**禁止盲目全量重跑**（一套 20+ 用例串行跑一次 40s+，反复全量重跑严重浪费时间）。正确姿势：① 先用 `-g "用例名"` 只跑失败的单个用例复现；② 用 curl/DB 直查等**最小手段快速定位根因**（如本例用 curl 手动复现 + 查 internal 端点返回，秒级定位到 JS `Number()` 精度丢失），避免在完整套件里盲猜；③ 修复后**只跑该用例验证**，最后才做一次全量回归确认无回归。写测试代码时同样先单点验证再全量。
- **测试产物清理约定（2026-08-12 起，全局纪律）**：每次 E2E/API 测试结束必须及时清理临时产物——前端 `test-results*` 目录、工作区 `.zcode/` 下已结束任务产生的临时计划/输出、`%TEMP%` 下 `sql_e2e*.ps1`/`verify_fix*.ps1`/`body_*.json`/`login*.json`/`create_*.json`/`export_*.json` 及 Sprint 临时脚本。只删除已确认属于本次任务且不再使用的文件，不要删除 `.zcode/` 下仍在使用的配置或活动会话文件。`e2e/sprint10/` 下的正式测试套件（如 `sql-console.spec.ts`）属源码，不清理。测试创建的数据库用户（如 `analyst_test`）清理前先问用户。

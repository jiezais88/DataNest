...
| F2 二次增强（2026-08-10，用户 6 项反馈）                          | 列表：去定时轮询（手动/操作后刷新）+ 运行时长列 + 创建人/创建时间/修改人/修改时间 4 列 + 详情抽屉（全角色）；向导：双栏等高各自滚动（右栏摘要+配置说明填充）+ 高级配置（并行度 1~8 / checkpoint 间隔 ≥3s，configJson 下发）；后端：V1.2.0 `started_at` + SystemUserApi 用户名回填 + configJson 校验与 YAML/提交侧下发（**坑：CDC YAML pipeline 段任意键不生效**，checkpoint interval 走 FlinkJobService 提交侧 flinkConfig 覆盖；并行度 >1 受集群 1 slot 限制，已记 gotchas） |

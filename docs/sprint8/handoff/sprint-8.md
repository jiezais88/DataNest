...
| F3 前端代码（2026-08-11，新增/修改）                              | `types/quality-report.ts` + `api/quality-report.ts`（8 API）；`pages/governance/quality-report/`（index.tsx 一屏 Dashboard：KPI×5/四档趋势/评分分布环图/数据源对比/评分趋势/问题清单 TOP5+抽屉分页；charts.tsx 手写 SVG 四图；constants.ts 判定中文标签）；路由 `/governance/quality-report` + Sidebar「质量报告」（查看 ALL_ROLES，导出治理员/超管）。后端补：summary 加 severe/warning 计数 + `score-distribution`/`datasource-comparison` 两端点 + `QualityScoreMapper.selectDatasourceScoreComparison`。**Review（With fixes）修复**：① 三处评分聚合统一只统计 ONLINE 表（OFFLINE 残留不计）；② 筛选草稿/应用模型统一（appliedRequest，草稿不再即时刷新半屏）；③ 抽屉白请求门控、图表 token 化、问题清单补类型列等 Minor |
| F2 二次增强（2026-08-10，用户 6 项反馈）                          | 列表：去定时轮询（手动/操作后刷新）+ 运行时长列 + 创建人/创建时间/修改人/修改时间 4 列 + 详情抽屉（全角色）；向导：双栏等高各自滚动（右栏摘要+配置说明填充）+ 高级配置（并行度 1~8 / checkpoint 间隔 ≥3s，configJson 下发）；后端：V1.2.0 `started_at` + SystemUserApi 用户名回填 + configJson 校验与 YAML/提交侧下发（**坑：CDC YAML pipeline 段任意键不生效**，checkpoint interval 走 FlinkJobService 提交侧 flinkConfig 覆盖；并行度 >1 受集群 1 slot 限制，已记 gotchas）。**2026-08-10 追加**：向导卡片加宽 1280；高级配置再开放 **表结构变更策略 schemaChangeBehavior（EVOLVE/LENIENT/EXCEPTION，YAML pipeline 段官方键）+ 快照分块大小 scanChunkSize（source 段 scan.incremental.snapshot.chunk.size，16~1048576）**，校验/下发/详情展示已 curl + 截图实测 |
| F2 向导布局定稿（2026-08-10）                                     | 调研后给用户 A/B/C 三方案（A 居中窄卡 / B 左竖步骤 / C 管道流向双卡），用户先选「先 A 后 C」，A 落地后又明确 **上 C 定稿**：4 步压 3 步（基本信息 / 配置管道 / 确认启动），配置管道步为**源卡 →（CDC 箭头）→ 目标卡**双卡布局（源卡：数据源/库/表勾选；目标卡：目标库 AutoComplete/表映射+主键自动回填，勾表实时联动）+ 底部横向配置带（同步模式/启动位点/写入模式/高级配置 2×2 网格）；居中卡片 max-w-1280，整页滚动；底部操作条与卡面同色（bg-root 会与页面背景融为一体，已去）+ border-t 分隔；截图验证通过 |
...
| `docs/sprint8/测试用例清单.md`（2026-08-11 新增）                     | 人工验收清单（对齐 sprint7 格式，按用户反馈精简）：**24 用例**（F1 8 / F2 8 / F3 5 / 权限与导出专项 3），只做核心链路人工确认，边缘场景由自动化覆盖；含环境速查、token 获取、幂等重播 SQL（附录 A，已实测可执行）；**验收种子数据已由测试会话直接执行完毕**（s7 基础数据 + F3 报告数据 + F2 CDC 源表） |
| 人工验收反馈修复（2026-08-11，用户验收后反馈 5 项）                   | ① chip 跳转筛选经生产 live 复现**确认可用**（用户误解源于列表页 chip 不可点——已补列表标签 chip 点击筛选，stopPropagation 防触发行点击）；② **三个导出 CSV→xlsx**（POI `poi-ooxml`，注意 `poi-ooxml-lite` 只是 schema jar 不含工作簿类）：`XlsxExportHelper`（SXSSF 流式 + 内容估算列宽，CJK 记 2 宽封顶 60，不用 autoSizeColumn）+ 中文枚举/时间格式保留 + 字符串单元格无公式注入面，`CsvExportHelper` 删除，conventions §8 改写；③ **Doris catalog 自动刷新**：app-job 新增 `dorisCatalogAutoRefreshHandler`（每 30s），realtime 内部端点仅 RUNNING 管道存在时才 REFRESH（用户约定：定时不走 @Scheduled 走 app-job）；④ CDC 运行时长自适应（N 秒/N 分 N 秒/N 小时 N 分/N 天 N 小时）；⑤ 质量报告查询/导出按钮固定宽度消抖（草稿+查询模型保留，用户确认）；**追加**：E2E 复跑发现 **PG 复制槽泄漏**（删除管道不清槽 → 残留槽占满 `max_wal_senders` → 新 PG 作业全部起不来），已加删除级联清槽（fail-open）+ 手工清存量 8 槽/取消孤儿作业，E2E 槽清理断言通过 |

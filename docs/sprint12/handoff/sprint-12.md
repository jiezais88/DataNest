...
| F3 开源物料 | [~] 截图待补 | 2026-08-17：Apache-2.0 `LICENSE`（Copyright 2026 DataNest Contributors）✅；根 `README.md` 整体重写 ✅（能力全景按 Sprint 0~11 实际交付 / 技术栈 Java 25+Boot 4+PowerJob+Flink CDC / 三层目录结构 / 快速开始指向 deploy.sh + docs/deploy.md / License 声明 + 默认密码提示，过时表述全部消除）；`CHANGELOG.md` v1.0.0 ✅（Keep a Changelog，按能力域分节覆盖 Sprint 0~12）。**遗留**：用户拍板截图暂缓——发布前需补 4~6 张实拍截图（存 `docs/screenshots/`）并在 README 补「界面截图」区块 |
| F4 GitHub 发布 | [~] 进行中 | 2026-08-17：① 仓库瘦身（用户拍板方案③ jar 移出 git 挂 Release 附件）：`scripts/fetch-flink-libs.sh`（10 jar Release URL + sha256 校验，已存在跳过，离线可重跑）+ `.gitignore` 加 `docker/flink/lib/*.jar` + deploy.sh 构建前自动 fetch + filter-branch 重写 177 提交剔除 jar，**size-pack 158MB → 9.5MB**；② **插曲**：filter-branch 结尾 reset --hard 把本地 182MB jar 也删了（index-filter 同时误删 .gitkeep 提交），已从运行中的 flink-jobmanager 容器 `docker cp` 逐个恢复，**10 个 sha256 与入库前实测值全部一致**，镜像重建验证通过；③ 远程 origin 已配置 https://github.com/jiezais88/DataNest.git（本地分支已改 main）；④ 首次 push 因凭据管理器登录框被取消失败（`User cancelled dialog`），待重推。后续：push main → tag v1.0.0 → Release（10 个 jar 作附件 + Release Notes）；截图待补 |

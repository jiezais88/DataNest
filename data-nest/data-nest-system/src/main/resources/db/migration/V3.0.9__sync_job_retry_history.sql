...
COMMENT
ON COLUMN sync_job_history.parent_history_id IS '父历史记录ID，重试时指向来源执行记录';
COMMENT
ON COLUMN sync_job_history.retry_count IS '当前执行链已发生的重试次数';
COMMENT
ON COLUMN sync_job_history.next_retry_at IS '计划下次重试时间（仅记录）';
...
COMMENT
ON COLUMN sync_job.last_execute_time IS '最近执行时间';
COMMENT
ON COLUMN sync_job.last_history_id IS '最近一次执行历史ID';

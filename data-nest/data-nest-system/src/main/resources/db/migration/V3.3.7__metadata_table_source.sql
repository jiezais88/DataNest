...
COMMENT
ON COLUMN metadata_table.task_source_type IS '任务来源类型：SQL / SYNC / PYTHON';
COMMENT
ON COLUMN metadata_table.source_dag_id IS '来源 DAG ID';
COMMENT
ON COLUMN metadata_table.source_dag_name IS '来源 DAG 名称';
COMMENT
ON COLUMN metadata_table.source_node_id IS '来源节点 ID';
COMMENT
ON COLUMN metadata_table.source_node_name IS '来源节点名称';

...
COMMENT
ON COLUMN metadata_table.last_collect_history_id IS '最近一次采集历史记录ID';
...
ALTER TABLE metadata_column
    ADD COLUMN IF NOT EXISTS last_collect_history_id BIGINT DEFAULT NULL;
COMMENT
ON COLUMN metadata_column.nullable IS '是否允许为空';
COMMENT
ON COLUMN metadata_column.last_collect_history_id IS '最近一次采集历史记录ID';

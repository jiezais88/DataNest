...
COMMENT
ON COLUMN metadata_table.source_type IS '元数据来源：BUILTIN_DORIS 内置Doris，EXTERNAL 外部数据源';
COMMENT
ON COLUMN metadata_column.source_type IS '元数据来源：BUILTIN_DORIS 内置Doris，EXTERNAL 外部数据源';
COMMENT
ON COLUMN datasource_connection.auto_collect_on_save IS '保存后是否自动采集元数据（0-否 1-是）';

...
export default function QualityScoreBadge({score, healthLevel, compact = false}: QualityScoreBadgeProps) {
    ...
    return (
        <div className="flex items-center gap-ds-2">
            {hasScore && cls ? (
                <>
                    <span className={`text-ds-subhead font-bold ${cls.text}`}>{score}</span>
                    <span className={`inline-flex items-center rounded-full px-ds-2 py-0.5 text-ds-small font-semibold ${cls.badge}`}>
                        {QUALITY_HEALTH_LABEL[level] ?? level}
                    </span>
                </>
            ) : (
                <>
                    <span className="text-ds-subhead font-bold text-[#94a3b8]">—</span>

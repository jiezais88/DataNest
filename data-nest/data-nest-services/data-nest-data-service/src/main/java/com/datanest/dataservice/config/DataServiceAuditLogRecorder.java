...
/**
 * data-service 服务的审计写入器（Sprint 11 F1）。
 * <p>
 * 经 SystemAuditApi Feign 调 system internal 审计写入端点；写入失败 fail-open（只打 warn 不阻断业务）。
 */
@Component
public class DataServiceAuditLogRecorder implements AuditLogRecorder {

    private static final Logger log = LoggerFactory.getLogger(DataServiceAuditLogRecorder.class);

    private final SystemAuditApi systemAuditApi;

    public DataServiceAuditLogRecorder(SystemAuditApi systemAuditApi) {

...
/**
 * 数据源表预览服务。
 * 微服务化 4.3：由 task-core-governance 迁入 engineering（唯一调用方 DataPreviewController 在本服务）。
 * 连接信息经 {@link EngineeringDatasourceApi} Feign 读取（engineering 容器内为 lb:// 自调用；
 * fail-fast，预览是同步用户操作，连接读不到直接报错，不降级）。
 */
@Service

 ...
 */
@Schema(description = "Python 质量规则脚本试跑请求")
@Data
public class QualityPythonScriptTestRequest {

    @Schema(description = "目标表 metadata_table.id（连接注入与 read_table 目标）", example = "1234567890123456789")
    @NotNull(message = "目标表不能为空")
    private Long tableId;

    @Schema(description = "Python 脚本（def check(df) 返回 dict）")

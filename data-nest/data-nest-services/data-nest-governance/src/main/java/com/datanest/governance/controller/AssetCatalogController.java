 ...
 */
@Tag(name = "数据资产目录", description = "资产多维搜索 / 分类体系维护 / 分类与负责人分配")
@RestController
...
public class AssetCatalogController {
    ...
    @Operation(summary = "分类浏览资产列表", description = "分页；uncategorized=true 查未分类；sort=score 按质量分降序；healthLevel 按健康度筛选")
    @GetMapping("/browse")
    public Result<PageResult<AssetSearchItemDTO>> browse(@Parameter(description = "数据域名称") @RequestParam(required = false) String domain,
                                                         ...
                                                         @Parameter(description = "排序方式（score=按质量分降序）") @RequestParam(required = false) String sort,

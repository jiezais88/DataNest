...
/**
 * 批量同步任务告警配置快捷入口（Sprint 5）。
 * 与全局告警中心操作同一数据源（alert_rule），任何入口修改实时同步。
 * 权限（PRD §8）：查看 = 超管/工程师/治理员；编辑 = 超管/工程师。
 */
@RestController
@RequestMapping("/sync-jobs")
public class SyncJobAlertRuleController {

    private final AlertRuleService alertRuleService;

    public SyncJobAlertRuleController(AlertRuleService alertRuleService) {
    ...
    public Result<AlertRuleDTO> getAlertRule(@PathVariable Long id) {
        return Result.ok(alertRuleService.getRuleByObject(AlertConstants.OBJECT_TYPE_SYNC_JOB, id));
    }
    ...
    public Result<AlertRuleDTO> updateAlertRule(@PathVariable Long id, @RequestBody AlertRuleDTO dto) {
        return Result.ok(alertRuleService.upsertRuleByObject(AlertConstants.OBJECT_TYPE_SYNC_JOB, id, dto));
    }
}

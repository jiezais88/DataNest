...
public class QualityJobUpdateRequest {
    ...
    @Schema(description = "任务描述（传 null 即清空）")
    ...
    @Schema(description = "执行超时阈值（分钟）；null/不传 = 不更新该字段（保留原值），>0 才生效（0 视为禁用超时）")
    @Min(value = 0, message = "超时阈值必须 ≥ 0")
    private Integer timeoutMinutes;

    @Schema(description = "引用的质量规则 ID 集合（全量覆盖关联）")

...
COMMENT ON COLUMN compliance_check_result.standard_id IS '命中的命名规范ID，未命中时可为空';
COMMENT ON COLUMN compliance_check_result.datasource_id IS '数据源ID';
COMMENT ON COLUMN compliance_check_result.database_name IS '数据库名';
COMMENT ON COLUMN compliance_check_result.schema_name IS 'Schema名';

-- ============================================
-- init-powerjob-db.sql
-- PowerJob 5.1.2 server 元数据库初始化（新环境免手工建库）
-- 说明：建库 + 授权 nacos 用户 + 官方 14 张表（来源 PowerJob others/powerjob-mysql.sql）
-- 注意：initdb 仅在 MySQL 数据卷首次创建时执行；已有环境如缺表需手动执行本文件。
-- ============================================

CREATE DATABASE IF NOT EXISTS powerjob DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
GRANT ALL PRIVILEGES ON powerjob.* TO 'nacos'@'%';
FLUSH PRIVILEGES;
